...
isOutsideScreen o = pos o < screenLeft || pos o > screenRight
    where
        pos = fst . getPosition
...
updateLife _ u
  | collisionMulti cow (mapGoodBirds map) = u 
    { universeLife = (life - 1) 
    , universeMap = collisionHandle map cow
    }
  | collisionMulti cow (mapBadBirds map) = u 
    { universeLife = (life - 2) 
    , universeMap = collisionHandle map cow
    }
  | collisionMulti cow (mapClovers map) = u 
    { universeLife = (life + 1) 
    , universeMap = collisionHandle map cow
    }
  | otherwise = u
